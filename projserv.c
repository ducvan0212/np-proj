...
int main(int argc, char **argv)
{
  ...
  int remaining_players[TOTAL_PLAYERS + 4] = {0,0,0,0,1,1,1};
      ...
      if (FD_ISSET(sockfd, &rset)) {
        ...
        if ( (n = Read(sockfd, buf, MAXLINE)) == 0) { /* connection closed by client */
          ...
          if (buf[0] == 'y') {
            ...
            {
              // truong hop 99 nguoi cung choi tra loi
              Writen(sockfd, after_anwsering_message, strlen(after_anwsering_message)+1);
            } else {
              // truong hop nguoi choi chinh tra loi
